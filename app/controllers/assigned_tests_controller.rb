class AssignedTestsController < ApplicationController
  ...
  def create
    @assigned_test = AssignedTest.new(assigned_test_params)

    respond_to do |format|
      if @assigned_test.save
        format.html { redirect_to profile_user_path(@assigned_test.user_id), notice: 'Assigned test was successfully created.' }
  ...
  private
    ...
    def assigned_test_params
      params.require(:assigned_test).permit(:user_id, :test_id, :test_mode_id)
    end
end
